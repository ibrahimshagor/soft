        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500">
              {language === 'bn' ? 'সকল সক্রিয় ব্যাংক, ক্যাশ ও এমএফএস একাউন্ট' : 'All active Bank, Cash, and MFS accounts'}
            </span>
            <button
              onClick={handleOpenAddAccount}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {accounts.map((acc) => (
              <div
                key={acc.id}
                className="p-5 rounded-3xl bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700/80 shadow-xs hover:shadow-md transition-all flex flex-col justify-between relative group"
              >
                <div>
                  {/* Card Header with Icon, Name, Type & Default tag */}
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {/* Quick Edit & Delete Icons in top corner */}
                  ...
                  {/* Detailed Info Card (Bank, Branch, Account Holder, A/C No) */}
                  <div className="mt-3 p-3 rounded-2xl bg-slate-50/70 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 space-y-1.5 text-xs">
                    {/* Bank Name */}
                    {acc.bankName && (
                      <div className="flex items-center justify-between text-[11px]">
                        <span className="text-slate-400">{language === 'bn' ? 'ব্যাংক:' : 'Bank:'}</span>
                    ...
                    {acc.accountHolder && (
                      <div className="flex items-center justify-between text-[11px]">
                        <span className="text-slate-400">{language === 'bn' ? 'হোল্ডার:' : 'Holder:'}</span>
                    ...
                    {/* Account / Mobile Number with Inline Copy */}
                    {acc.accountNumber && (
                      <div className="flex items-center justify-between text-[11px] pt-0.5">
                        <span className="text-slate-400">
                          {acc.type === 'bank' ? (language === 'bn' ? 'একাউন্ট নং:' : 'A/C No:') : (language === 'bn' ? 'মোবাইল নং:' : 'Wallet / No:')}
                        </span>
                        <div className="flex items-center gap-1.5 font-mono font-bold text-slate-900 dark:text-white">
                          ...
                          <button
                            ...
                            className="p-1 rounded-md bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 hover:text-amber-600 transition-colors"
                          >
                            {copiedAccId === acc.id ? (
                    ...
                    {acc.routingNumber && (
                      <div className="flex items-center justify-between text-[11px]">
                        <span className="text-slate-400">{language === 'bn' ? 'রাউটিং নং:' : 'Routing No:'}</span>
                        <span className="font-mono text-slate-700 dark:text-slate-300">{acc.routingNumber}</span>
                      </div>
                    )}
                  </div>
                </div>

                {/* Card Action Buttons (Copy Details, WhatsApp, Edit, Statement) */}
                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-700/60 space-y-2">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button
                      ...
                      className="py-1.5 px-2.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300 font-bold flex items-center justify-center gap-1.5 text-xs transition-colors shadow-2xs"
                    >
                    ...
                    <button
                      ...
                      className="py-1.5 px-2.5 rounded-xl bg-slate-100 dark:bg-slate-700/60 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold flex items-center justify-center gap-1.5 text-xs transition-colors"
                    >
                      {copiedAccId === acc.id ? (
                        <>
                          <Check className="w-3.5 h-3.5 text-emerald-600" />
                          <span className="text-emerald-600">{language === 'bn' ? 'কপি হয়েছে!' : 'Copied!'}</span>
                        </>
                      ) : (
                        <>
                          <Copy className="w-3.5 h-3.5" />
                          <span>{language === 'bn' ? 'কপি করুন' : 'Copy Info'}</span>
                        </>
                      )}
                    </button>
                  </div>

                  <div className="flex items-center justify-between gap-1.5 text-xs pt-1">
                    <button
                      type="button"
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => handleOpenEditAccount(acc)}
                        className="px-2.5 py-1.5 rounded-xl text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950/40 font-bold flex items-center gap-1 transition-colors"
                      >
                      ...
                      <button
                        type="button"
                        onClick={() => handleDeleteAccount(acc)}
                        className="px-2 py-1.5 rounded-xl text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 font-bold flex items-center gap-1 transition-colors"
                      >
